...
- Maximum supported historical lookback
- Pagination for large history requests
- Duplicate or overlapping records across current-rate polls
- Timestamp meaning
- How missing or stale current-rate data is represented

## Initial ingestion decisions

- Poll current rates every 10 minutes.
- Store raw API responses before filtering or transforming them.
- Reserve part of the monthly quota for retries and historical backfill.
- Test the history endpoint before defining a larger backfill strategy.